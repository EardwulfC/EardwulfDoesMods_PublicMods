...
namespace MassFarming
{
    ...
    public class MassPlant
    {
        ...
        public static void UpdatePlacementPostfix(Player __instance, bool takeInput, float dt)
        {
            ...
            if (!MassFarming.NoActionKeyNeeded.Value && !Input.GetKey(MassFarming.ControllerPickupHotkey.Value.MainKey)
                    && !Input.GetKey(MassFarming.MassActionHotkey.Value.MainKey))
            {
                //Hotkey required
                return;
            }
